...
export const HUD: React.FC<HUDProps> = ({ avatarUrl }) => {
  const { health, hunger, cameraMode, setCameraMode } = useGameStore();
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-2 p-2 bg-zinc-900/40 rounded-2xl border border-white/5 backdrop-blur-sm self-start pointer-events-auto">
        {(['1P', '2P', '3P'] as const).map((mode) => (
          <button
            key={mode}
            onClick={() => setCameraMode(mode)}
            className={`px-3 py-1.5 rounded-xl text-[10px] font-black mono transition-all ${cameraMode === mode
